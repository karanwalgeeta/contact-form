...
app.post("/contact", async (req, res) => {
  try {
    ...
    // 1. Admin ko mail — tumhare paas aayega
    const adminMailOptions = {
      from: `"Website Contact" <${process.env.SMTP_FROM_EMAIL}>`,
      to: process.env.RECIPIENT_EMAIL, // alpineodyssey.bt@gmail.com — fixed
    ...
    // 2. Client ko mail — jo bhi form bhare usko Thank You jayega
    const clientMailOptions = {
      from: `"Alpine Odyssey" <${process.env.SMTP_FROM_EMAIL}>`,
      to: email, // user ne jo email diya form mein
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 8px;">
          ...
          <div style="margin-top: 30px; padding-top: 15px; border-top: 1px solid #e0e0e0; color: #888; font-size: 12px;">
            <p>Best Regards,<br/><b>Alpine Odyssey</b></p>
          </div>
        </div>
      `
    };

    // Dono mails ek saath bhejo
